# Brief: DinoStack Onboarding Crashcourse

**Problem:** Engineers new to AI-assisted development skip the up-front spec/planning step that most determines output quality, and have no low-stakes, hands-on way to learn to drive the dinostack framework. There is no guided environment where they can practice spec-first planning and run dinostack end-to-end against realistic tasks of escalating difficulty.

**Success criteria:**
- A forkable repo whose root README is a self-contained guide - orients the engineer, walks dinostack install/setup, and teaches the spec-first workflow (the spec-first `/implement-ticket` loop, conductor/worker/skeptic roles, risk classification) well enough to start unaided.
- Three tier branches (`intro`, `intermediate`, `advanced`), each a runnable Next.js + TS app carrying one `TASK.md` that exercises a distinct dinostack flow: fix-broken-code-to-PR, feature/integration, and Figma design-to-code.
...
- Not a rewrite or fork of the dinostack framework itself - the crashcourse consumes dinostack, it does not modify it.
- Not a production app; the sample app exists only to host the exercises.

**Constraints:**
- dinostack brand in prose; reference real current command names - everyday user commands are `/init-project`, `/implement-ticket`, `/wrap`, while `/brief` and `/skeptic` are framework-run (conductor-invoked) commands framed as such; README notes install is via the agentic-engineering package.
...
manual_smoke: >
  ...
  from zero (including dinostack install), and confirms they can start each of the three
...
- **Branding:** "dinostack" brand in prose; real current command names; install via agentic-engineering until the rename ships.
